...
def _validate(params: dict) -> None:
    ...
    if params.get("chamfer_type", 1) not in (1, 2):
        raise SketchEditError(
            "sketch_chamfer: chamfer_type must be 1 (DistanceDistance) or 2 (DistanceEqual)"
        )
...
def _apply(doc: Any, sk: Any, params: dict) -> dict:
    ...
    d2 = mm_to_m(
        params.get("dist2_mm", params["dist1_mm"])
    )  # CreateChamfer 3rd arg = 2nd distance
...
def _verify(before: int, after: int, params: dict) -> tuple[bool, str]:
    return (
        after > before,
        f"segments {before}->{after} (chamfer inserts a line, net +1)",
    )
