...
class TestCollectPins:
    def test_deduplicates_repeated_rows(self, tmp_path):
        contrib = tmp_path / "CONTRIBUTING.md"
        contrib.write_text(
            textwrap.dedent(
                """\
                ...
                | --- | --- | --- | --- | --- | --- | --- |
                | `src/ai_sw_bridge/errors/circuit_breaker.py` | SolidworksMCP-python | MIT | a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2 | 2026-06-01 | TBD | first port |
                | `src/ai_sw_bridge/com/executor.py` | SolidworksMCP-python | MIT | a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2 | 2026-06-02 | TBD | second port, same pin |
            """
            ),
            encoding="utf-8",
        )
        with patch("check_upstream_drift.CONTRIBUTING", contrib):
            pins = collect_pins()
        # Same repo + same SHA across two rows — deduped to 1
        assert len(pins) == 1
...
class TestMain:
    ...
    def test_no_pins_exits_0(self, tmp_path):
        with patch("check_upstream_drift.CONTRIBUTING", tmp_path / "nope.md"):
            exit_code = main([])
        assert exit_code == 0
