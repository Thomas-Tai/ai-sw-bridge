...
class TestReadPinsFromHarvestPlan:
    ...
    def test_skips_recipe_without_sha(self, tmp_path):
        ...
        pins = read_pins_from_harvest_plan(hp)
        # Pin is present but has empty SHA — still returned for the repo
        assert len(pins) == 1
        assert pins[0].pinned_sha == ""
...
class TestCollectPins:
    def test_deduplicates_across_sources(self, tmp_path):
        ...
        with patch("check_upstream_drift.HARVEST_PLAN", hp), patch(
            "check_upstream_drift.CONTRIBUTING", contrib
        ):
            pins = collect_pins()
...
class TestCheckDrift:
    def test_identical_returns_zero(self):
        pin = UpstreamPin(
            repo="owner/repo", pinned_sha="a" * 40, source_file="test"
        )
    ...
    def test_missing_sha_returns_error(self):
        pin = UpstreamPin(
            repo="owner/repo", pinned_sha="", source_file="test"
        )
        result = check_drift(pin)
...
class TestOutputFormats:
    def test_table_has_headers(self):
        results = [DriftResult(repo="owner/repo", pinned_sha="a" * 40, commits_since_pin=5)]
...
class TestMain:
    def test_exits_1_when_drift_exceeds_threshold(self, tmp_path):
        ...
        with patch("check_upstream_drift.CONTRIBUTING", contrib), patch(
            "check_upstream_drift.HARVEST_PLAN", tmp_path / "nope.md"
        ), patch(
            "check_upstream_drift._github_get", return_value=(mock_data, "")
        ):
    ...
    def test_no_pins_exits_0(self, tmp_path):
        with patch("check_upstream_drift.CONTRIBUTING", tmp_path / "nope.md"), patch(
            "check_upstream_drift.HARVEST_PLAN", tmp_path / "nope2.md"
        ):
            exit_code = main([])
        assert exit_code == 0
