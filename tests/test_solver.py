...
def _run(sense_seq, *, read_val=0.010, drive_side=None, **kwargs):
    ...
    with patch.object(S, "wrapper_module", return_value=object()), \
...
def test_fail_closed_reverts_to_original():
    ...
    res, drive_m, _ = _run(
        [(1, 4000.0), (1, 3800.0), (1, 3600.0)], read_val=0.010,
        step_mm=2.0, max_iters=3)
...
def test_drive_failure_is_surfaced_and_reverted():
    ...
    res, drive_m, _ = _run(
        [(1, 4000.0)], read_val=0.010, step_mm=2.0, drive_side=flaky)
    assert res["ok"] is False
