...
| Spec type handler | Non-sketch: function in the matching `src/ai_sw_bridge/spec/handlers/<family>.py` leaf (dress_up, extrude, hole, pattern, revolve, sketch — **not** `builder.py`). Sketch: `SketchHandler` subclass in `src/ai_sw_bridge/spec/sketches/` | Non-sketch: implement `_build_<type>` in the family leaf, re-export it into `builder.py` (`from .handlers.<family> import _build_<type>`) so `_wire_handlers()` resolves it, wire it into the `handlers` dict inside `_wire_handlers()`, and add a `FeatureType(...)` entry to `DESCRIPTORS`. Sketch: export the class from `spec/sketches/__init__.py` and wire `Handler().build` the same way via a `_build_sketch_<type>` adapter. | `_build_<type>(ctx: BuildContext, feat: dict[str, Any]) -> BuiltFeature` (non-sketch) or `SketchHandler.build(self, ctx, feat) -> BuiltFeature` (sketch, template method over `_enter_sketch`/`_draw_geometry`/`_add_dimensions_inline`/`_record_deferred_dimensions`/`_finalize`) — both in `spec/_build_context.py` | Schema test in `tests/test_schema.py` + reference-check test in `tests/test_validator.py`; `tests/test_extension_conformance.py::test_every_spec_handler_lives_in_handlers_package` (no handler may resolve to `builder.py`) + the import-linter handler-leaf contract; the type string must be in `schema.py`'s `ALL_TYPES` union. |
...
- `CONTRIBUTING.md` — "Adding a capability" (the two-registry distinction plus
  the spec-build handler's step-by-step recipe, with the schema/validator/example
  steps this table doesn't repeat).
