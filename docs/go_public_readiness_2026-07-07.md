...
- **Dependabot PRs — DEFERRED (decision 2026-07-07).** The only non-`master` branches are 5 Dependabot dependency-bump PRs (#2 action-gh-release 2→3, #3 gitleaks-action 2→3, #4 checkout 5→7, #11 upload-artifact 4→7, #5 black 25.12.0→26.5.1). **Not merged** — they can't be CI-validated while Actions is down, and two would risk the post-flip green (black #5 bumps only the `pyproject` pin, not the hard-coded `black==25.12.0` in `ci.yml`/`release.yml` → mismatch + whole-tree reformat; the Action major bumps can change `release.yml`/`security.yml` behavior). They are routine maintenance, **not go-public blockers.** **Post-flip plan:** once Actions runs again, let each PR's own CI validate it, apply black's reformat with the pin, and merge the green ones.
