...
| G-05 | `FeatureRevolve2(IsCut=True)` returns None for a Front-plane groove whose swept torus intersects the hub (lint exit 0) | UNVERIFIED. Seat-free arithmetic: groove r[14.0, 15.5] at X[-2, 2] is inside the hub wall r[12, 16] at X[-6, 6] — not empty air, not a crossed centerline, not the opposite-side authoring rule. Remaining candidates: ReverseDir (spec `flip`) and UseFeatScope=True with sketch-only selection on a plate+hub part (pinned, spec-unreachable). | Seat A/B: `probe_46_revolve_cut.json` (defaults) vs `probe_46_revolve_cut_flip.json` (`flip: true`). If both None, next experiment is UseFeatScope=False in `_feature_revolve2_args`. | Pre-flight honest-skips `revolve_*` (`PREFLIGHT_SKIP_CODE` → `coverage.complete=false`, `--lint --strict` exit 8). COM args unchanged until a seat confirms. | issue #46 |
