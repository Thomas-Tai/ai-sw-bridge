...
> **Correction (folded in during Task 7, 2026-08-08) — the table above overclaims what an `ai-sw-build` part spec can declare.** The historical text above is left intact for the record; here are the verified facts. `draft`, `shell` (line 94), and offset `ref_plane` (line 94) are `feature_add_kinds` — reachable **only** via `ai-sw-mutate` against a live, already-built doc — they are **not** `spec_feature_types`, so they cannot be declared as a feature in an `ai-sw-build` part spec (`demo_baseplate`'s draft, line 92, and `demo_bearing_block`'s shell + offset reference plane, line 94, are therefore not buildable that way either). `counterbore`, `countersink` (line 94), and cosmetic thread (line 93) are absent from all 67 kinds entirely (`ai-sw-build --list-kinds` has no such entries; `simple_hole`'s `end_condition` does not include a counterbore/countersink mode). The "≈16–18 feature families" aggregate (line 96) is accordingly too high. **The as-built widget uses 20 real build families** across the three parts — baseplate 9 / shaft 4 / block 7 — see `examples/demo_widget/README.md` for the authoritative, spec.json-derived list. The mutate-only kinds (`draft`, `shell`, `ref_plane`), if shown in the demo at all, appear via the `ai-sw-mutate` chapter/beat against a live doc, never inside a part-build spec.
