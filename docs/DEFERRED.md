...
| **Ref Point (F0, Wave-5 partial) -- `SelectByID(VERTEX)` walls out-of-process** | Characterized on a live seat (SW2024 SP1, `spikes/v0_16/_run_f0_pae.py`). The `_create_ref_point` handler pipeline is wired, but its first step -- `doc.SelectByID("", "VERTEX", x, y, z)` to pick a model vertex -- returns `False` for every probed coordinate (all 8 box corners tested). `SelectByID2` on `IModelDocExtension` is not reachable with `Callout=None` (type mismatch); `GetBodies2`/vertex traversal requires `IPartDoc` QI which late-bound `IModelDoc2` doesn't expose cleanly from out-of-process Python. **The 3 sibling F0 kinds (ref_plane / ref_axis / coordinate_system) are seat-GREEN and re-advertised**; only ref_point remains gated (propose fails closed). Unblocking requires a Route-A typed `IPartDoc` QI pattern, a body-traversal vertex discovery (`IBody2.GetVertices`), or a VBA-emit-and-run detour. First raised 2026-06-01, Wave-5 seat session. |
...
| **Rib / Dome / Wrap (F3/F4/F5, Wave-5) -- Route-B legacy `Insert*` methods all wall at entity-select step** | Characterized on a live seat (SW2024 SP1, `spikes/v0_16/spike_{rib,dome,wrap}_v2.py`). The typelib-confirmed legacy signatures are **`fm.InsertRib(Bool, Bool, Double, Long, Bool, Bool, Bool, Double, Bool, Bool)` (10 args)**, **`doc.InsertDome(Double, Bool, Bool)` (3 args, on IModelDoc2 NOT IFeatureManager)**, and **`fm.InsertWrapFeature2(Long, Double, Bool, Long, Long)` (5 args)**. All three calls return `None` with status `OK` (no exception), but no feature materializes. **Root cause (shared with F0 ref_point):** the legacy `Insert*` recipes require pre-selection of a model entity (a face, an edge, or a sketch-on-face) via `doc.SelectByID("", "FACE", x, y, z)` or `ext.SelectByID2("", "FACE", x, y, z, Append, Mark, Callout, SelectOption)`. **Both APIs wall from out-of-process Python:** `SelectByID("FACE", ...)` returns `False` for every probed coordinate on a clean box; `SelectByID2(...)` rejects `Callout=None` with `Type mismatch` and rejects `pythoncom.Missing` with `Parameter not optional`. The three kinds are **not advertised** (propose fails closed). Unblocking needs a Route-A typed `IFace2` / `IEdge` selection pattern (body-traversal via `IPartDoc` QI -> `IBody2.GetFaces` -> select the returned dispatch), which is the same epic that unblocks F0 ref_point. First raised 2026-06-01, Wave-5 seat session. |
| **E1 Inertia -- tensor reads (`PrincipalAxesOfInertia`, `GetMomentOfInertia`) wall on VARIANT(array) marshaling** | `observe_inertia.py` patched (2026-06-01 seat session) to use `typed(ext, "IModelDocExtension").CreateMassProperty` as a property-get (the legacy late-bound `ext.CreateMassProperty()` raised `Member not found` on SW2024 SP1). The `IMassProperty` object is acquired, `Volume`/`SurfaceArea`/`Mass`/`Density`/`CenterOfMass` are readable (the scalar reads work out-of-process). But the two tensor reads wall: `PrincipalAxesOfInertia` and `GetMomentOfInertia` on `IMassProperty2` take a center-of-rotation `VARIANT(VT_ARRAY | VT_R8, [x,y,z])` arg. **Without the arg:** `Invalid number of parameters`. **With the arg (explicit `win32com.client.VARIANT`):** pywin32's dispatch layer chokes with `TypeError: int() argument must be a string, a bytes-like object or a real number, not 'VARIANT'` -- the VARIANT isn't being unwrapped at the COM boundary, so the callee sees the wrapper object instead of the SAFEARRAY. Same wall as the edge-flange `VARIANT(VT_DISPATCH, None)` case that was solved by the explicit wrapper pattern in `spikes/v0_16/spike_edgeflange.py`; unblocking E1 likely needs the mirror pattern for `VARIANT(VT_ARRAY | VT_R8, ...)`. Helper now fail-softs and records the error; scalar reads still work. First raised 2026-06-01, Wave-5 seat session. |
