...
| **Ref Point (F0, Wave-5) -- face-centroid path wired + gated; vertex-coord path still walls** | **Entity-select wall CRACKED 2026-06-01 (W5.3 Epic B, seat-GREEN `spikes/v0_16/spike_entity_select.py`, `880486a`).** The durable persist round-trip (`body.GetFaces() → GetPersistReference3 → GetObjectByPersistReference3 → typed(IEntity).Select2`) selects faces/edges/vertices live out-of-process — the same `resolve_manifest_face`/`select_entity` infra `wizard_hole`/`draft` already ship. With a face selected, `fm.InsertReferencePoint(4, 0, 0.0, 1)` (type 4 = `swRefPointTypeInCentreOfFace`) **materializes a centroid ref-point** (seat-proven). **`_create_ref_point` now wires this as the `target.face_ref` variant**, but `ref_point` stays **de-advertised** (absent from `_SUPPORTED_FEATURE_TYPES`; propose fails closed) **pending a production-handler PAE** on a seat (the F0 discipline: spike-materialize ≠ production-handler-validated). The legacy `target.point` vertex-coordinate path (`SelectByID("","VERTEX",x,y,z)` + type 5) **remains a wall** (SelectByID(VERTEX) returns False out-of-process) and is retained only as a non-advertised fallback. **Next step = seat PAE of the wired face-centroid handler → flip advertise.** First raised 2026-06-01, Wave-5 seat session; entity-select cracked 2026-06-01, Epic B. |
...
| **Rib / Dome / Wrap (F3/F4/F5, Wave-5) -- now feature-recipe walls (entity-select root cause SOLVED)** | Characterized on a live seat (SW2024 SP1, `spikes/v0_16/spike_{rib,dome,wrap}_v2.py`). The typelib-confirmed legacy signatures are **`fm.InsertRib(Bool, Bool, Double, Long, Bool, Bool, Bool, Double, Bool, Bool)` (10 args)**, **`doc.InsertDome(Double, Bool, Bool)` (3 args, on IModelDoc2 NOT IFeatureManager)**, and **`fm.InsertWrapFeature2(Long, Double, Bool, Long, Long)` (5 args)**. All three return `None` with status `OK` (no exception), no feature materializes. **The original "entity-select root cause" is now SOLVED** (W5.3 Epic B, `880486a`): the durable persist round-trip (`select_entity`) selects a live face out-of-process, and Epic B re-probed all three `Insert*` calls **with a face successfully selected** — they still return `None`. So these are now **feature-recipe walls** (wrong arg values or a missing precondition like a sketch-on-face), NOT selection walls. The three kinds stay **not advertised** (propose fails closed). Unblocking now needs a per-feature recipe spike (correct arg tuple + any required sketch-on-face), each its own small task — no longer blocked on the cross-cutting selection epic. First raised 2026-06-01, Wave-5 seat session; reclassified 2026-06-01, Epic B. |
