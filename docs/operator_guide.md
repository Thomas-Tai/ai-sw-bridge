...
|---|---|---|
| `ai-sw-probe` / `ai-sw-build`: *"command not found"* / *"not recognized"* | the virtual environment isn't active in this shell | re-run `.venv\Scripts\activate` (your prompt should show `(.venv)`), then retry — or re-do the install |
