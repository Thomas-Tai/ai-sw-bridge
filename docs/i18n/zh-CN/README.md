---
translated-from: 9d11889296f81fd30a5521a0e10765f4fceb5e5f
---
...
[![Platform](https://img.shields.io/badge/platform-Windows-lightgrey)](#面向操作者--5-分钟快速入门)
...
有两条路可走。**无论走哪一条，唯一的硬性前提都是 SOLIDWORKS 本身** —
已安装并正在运行（桥接器通过 COM 驱动它；这里的任何步骤都不会安装
SOLIDWORKS）。在 2024 SP1 上测试过；2021 SP5+ 亦可运行。

**A. 没有 Python？用 Windows 安装程序（最简单 — 不需要终端）。**
从[最新发布版本](https://github.com/Thomas-Tai/ai-sw-bridge/releases/latest)的
**Assets** 中下载 `ai-sw-bridge-setup-<version>.exe` 并双击运行。它**未经代码
签名**，所以在"Windows 已保护你的电脑"提示处点击 **More info → Run anyway**。
它内置了一个私有的 Python — 你**不**需要 Python、Git 或命令行 — 会替你运行
`pywin32` 步骤，并提供一个复选框来接入 MCP 服务器。然后打开一个新终端，前往
第 2 步。*（如果最新发布版本的 Assets 只显示 "Source code"，说明 `.exe` 还没有
附加到那个发布上 — 暂时改用路径 B。）*

**B. 更想用脚本化 / 终端方式（pipx）。** 面向开发者，或任何已经习惯在命令行
工作的人。你需要 64 位的 **Python 3.10+** 和 **Git** 位于你的 `PATH` 上
（SOLIDWORKS 是 64 位的，所以 32 位 Python 无法绑定），外加 `pipx`：
...
`[mcp]` extra（在上面的命令中）会拉取 MCP SDK，使 `ai-sw-mcp` 得以运行。
...
**Chat-first（推荐）— 直接和 Claude Desktop 对话，它会替你构建。** 这会把桥接器的
MCP 服务器接入 [Claude **Desktop** 应用](https://claude.ai/download)（安装版应用，
*不是*浏览器版），这样 Claude 就能自己调用构建工具 — 循环中不再需要终端：

```powershell
ai-sw-doctor --register     # writes the MCP server into Claude Desktop's config (timestamped backup first)
```

重启 Claude Desktop，然后直接开口：*"用 ai-sw-bridge 构建一个 40 × 30 × 10 mm 的板子，
四角各有一个 Ø5 mm 的通孔，距各边 5 mm。"* Claude 会起草规格、展示给你，并在写入任何
东西**之前请你批准**（elicitation 把关）。批准之后，零件就会构建。

**Copy-paste（任意 AI，无需 MCP）— 由你来执行命令。** 打开 Claude / ChatGPT /
Codex 并粘贴：

> 我正在使用 **ai-sw-bridge** — 一个让 AI 助手通过 COM API 驱动 SOLIDWORKS 的桥接器。在做任何事之前，请先阅读 **[`docs/AGENTS.md`](../../AGENTS.md)** — 它会告诉你规则、规格格式，以及在执行前哪些内容需要我确认。
>
> 我的目标：*在此描述你的零件 — 例如"构建一个 40 × 30 × 10 mm 的板子，四角各有一个 Ø5 mm 的通孔，距各边 5 mm。"*
>
> 请提出一份 JSON 规格供我审查，之后再执行 `ai-sw-build`。

代理会阅读 [`docs/AGENTS.md`](../../AGENTS.md)，起草一份规格，然后**停下来**等你审查；由你自己执行 `ai-sw-build`。同样是 propose → approve → execute 循环，只是从终端驱动。
...
| `ai-sw-mutate` | stable | 对 `*_locals.txt` 变量执行 propose → dry-run → commit（或 undo）变更。子命令：`propose` / `dry_run` / `commit` / `undo_last_commit`。提议持久化到 `./proposals/`（可通过 `AI_SW_BRIDGE_PROPOSALS` 覆盖）。 | ⚠️ 需批准 |
...
**当前发布版本：`v1.7.1` — 商业版，Production/Stable。** 一个公开 Python
...
SOLIDWORKS 是 Dassault Systèmes SE 的注册商标。`ai-sw-bridge` 是一个独立项目，**与
Dassault Systèmes 没有从属关系，也未获其授权或认可**。它通过已公开的 COM API 驱动
SOLIDWORKS 以实现互操作；所有其他商标均归其各自所有者所有。
