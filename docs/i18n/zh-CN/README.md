...
4. **CHM 是 API 签名的事实来源。** 当一个调用返回 `PARAMNOTOPTIONAL` 时，我们不会靠猜 — 我们从 `sldworksapi.chm` 重新提取，并在运行时断言参数数量。生成的参考文档（`api_reference.md`）没有被提交进仓库 — 可以从 [`tools/_api_extract_input.json`](../../../tools/_api_extract_input.json) 在本地重新生成它；已提交的超集是 [`docs/sw_api_full.md`](../../sw_api_full.md)。
