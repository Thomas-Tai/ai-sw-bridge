...
> ⚠️ **此翻译已过期，仅供参考。** 内容冻结于 `c8ce816`（约 v0.3 时期）：下方的 CLI 命令清单、功能范围、测试数量与项目状态均已严重落后。**许可证、兼容性与能力范围请一律以[英文 README](../../../README.md) 为准**（目前 v1.7，商业／专有许可）。完整同步追踪于 Issue #10。
...
架构与设计理念（为什么选择 JSON 规格而非流畅 API、分层模型），请阅读 [`docs/CLASS_RELATION_MAP.md`](../../CLASS_RELATION_MAP.md)。
...
│   └── CLASS_RELATION_MAP.md  # 类 / 外观 / 注册表 / COM 关系图
