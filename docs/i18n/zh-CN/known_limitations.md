...
> **Language**: [English](../../known_limitations.md) · 简体中文
...
机械层面：扩展 `_select_extrude_face` 以在拉伸轴为 `+/-Y` 或 `+/-X` 时（目前只接了 `+/-Z`）计算切平面偏移，并扩展面草图处理器中的 mirror-u 逻辑。估计：60-90 分钟，包含规格测试。已跟踪于 [CHANGELOG](../../../CHANGELOG.md) 的"近期"层级。
