...
> ⚠️ **此翻譯已落後於英文版。** 英文 [README](../../../README.md) 為權威版本；最近的更新（示範 GIF 套件與 README 重構）尚未同步至此翻譯。
...
[![Platform](https://img.shields.io/badge/platform-Windows-lightgrey)](#給操作者--5-分鐘快速入門)
...
有兩條路可走。**無論走哪一條，唯一的硬性前置條件都是 SOLIDWORKS 本身** —
已安裝並正在執行（橋接器透過 COM 驅動它；這裡的任何步驟都不會安裝
SOLIDWORKS）。在 2024 SP1 上測試過；2021 SP5+ 亦可運作。

**A. 沒有 Python？用 Windows 安裝程式（最簡單 — 不需要終端機）。**
從[最新發行版本](https://github.com/Thomas-Tai/ai-sw-bridge/releases/latest)的
**Assets** 中下載 `ai-sw-bridge-setup-<version>.exe` 並按兩下執行。它**未經程式碼
簽章**，所以在「Windows 已保護您的電腦」提示處點選 **More info → Run anyway**。
它內建了一個私有的 Python — 你**不**需要 Python、Git 或命令列 — 會替你執行
`pywin32` 步驟，並提供一個核取方塊來接上 MCP 伺服器。接著開啟一個新終端機，前往
步驟 2。*（如果最新發行版本的 Assets 只顯示「Source code」，表示 `.exe` 還沒有
附加到那個版本上 — 暫時改用路徑 B。）*

**B. 偏好腳本化／終端機方式（pipx）。** 給開發者，或任何已經習慣在命令列
工作的人。你需要 64 位元的 **Python 3.10+** 與 **Git** 在你的 `PATH` 上
（SOLIDWORKS 是 64 位元，所以 32 位元 Python 無法附加），外加 `pipx`：
...
`[mcp]` extra（在上方指令中）會拉入 MCP SDK，讓 `ai-sw-mcp` 得以執行。
`ai-sw-doctor`（下一步）會替你驗證 PATH 與這個 pywin32 步驟。
...
**Chat-first（推薦）— 直接和 Claude Desktop 對話，它會直接建構。** 這會把橋接器的
MCP 伺服器接上 [Claude **Desktop** 應用程式](https://claude.ai/download)（安裝版
應用程式，*不是*瀏覽器版），讓 Claude 能自己呼叫建構工具 — 迴圈中不再需要終端機：

```powershell
ai-sw-doctor --register     # writes the MCP server into Claude Desktop's config (timestamped backup first)
```

重新啟動 Claude Desktop，然後直接開口：*「用 ai-sw-bridge 建構一個 40 × 30 × 10 mm 的板子，
四角各有一個 Ø5 mm 通孔，距各邊 5 mm。」* Claude 會起草規格、展示給你看，並在寫入任何
東西**之前請你核准**（elicitation 閘門）。核准之後，零件就會建構。

**Copy-paste（任意 AI，不需要 MCP）— 由你來執行指令。** 開啟 Claude / ChatGPT /
Codex 並貼上：

> 我正在使用 **ai-sw-bridge** — 一個讓 AI 助理透過 COM API 驅動 SOLIDWORKS 的橋接器。在做任何事之前，請先閱讀 **[`docs/AGENTS.md`](../../AGENTS.md)** — 它會告訴你規則、規格格式，以及執行前有哪些事需要我確認。
>
> 我的目標：*在此描述你的零件 — 例如「建構一個 40 × 30 × 10 mm 的板子，四角各有一個 Ø5 mm 通孔，距各邊 5 mm。」*
>
> 請提出一份 JSON 規格供我審查，之後再執行 `ai-sw-build`。

代理會閱讀 [`docs/AGENTS.md`](../../AGENTS.md)，起草一份規格，然後**停下來**等你審查；由你自己執行 `ai-sw-build`。同樣是 propose → approve → execute 迴圈，只是從終端機驅動。
...
| `ai-sw-mutate` | stable | 對 `*_locals.txt` 變數變更執行 propose → dry-run → commit（或 undo）。子指令：`propose` / `dry_run` / `commit` / `undo_last_commit`。提案會持久化到 `./proposals/`（可用 `AI_SW_BRIDGE_PROPOSALS` 覆寫）。 | ⚠️ 需核准 |
...
**目前版本：`v1.7.1` — 商業版，Production/Stable。** 一個公開的 Python
