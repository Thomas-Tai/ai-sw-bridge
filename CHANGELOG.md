...
- **Published spec JSON Schema** (`schema/ai-sw-bridge.spec.schema.json`). A
  standalone `$schema` for editor autocomplete and inline validation of
  `spec.json` files — a serialization of the v1 `schema.SCHEMA` generated by
  `tools/emit_spec_schema.py` and kept in lockstep by a CI sync gate
  (`tests/test_spec_schema_published.py`: byte-identity + metaschema validity +
