...
4. **CHM is the source of truth for API signatures.** When a call returns `PARAMNOTOPTIONAL`, we don't guess — we re-extract from `sldworksapi.chm` and assert the arg count at runtime. The generated reference (`api_reference.md`) is not committed — regenerate it locally from [`tools/_api_extract_input.json`](tools/_api_extract_input.json); the committed superset is [`docs/sw_api_full.md`](docs/sw_api_full.md).
