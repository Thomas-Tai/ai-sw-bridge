...
`FeatureRevolve`, `FeatureChamferType`, `FeatureFillet*`, `InsertCutSwept5`, `InsertProtrusionSwept`, `FeatureCutThin2`, `FeatureBossThin2`, `SimpleHole3`, `InsertMirrorFeature`, `InsertLinearPatternFeature`. Add them to [`tools/_api_extract_input.json`](tools/_api_extract_input.json) and regenerate to expose them via `sw_types.py`.
