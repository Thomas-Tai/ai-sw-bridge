...
WT = os.path.join("C:" + os.sep, "D", "WorkSpace", "[Local]_Station", "01_Heavy_Assets", "aisw-W5.3")
sys.path.insert(0, os.path.join(WT, "src"))
RESULTS_DIR = os.path.join(WT, "spikes", "v0_16", "_results")
PART_TEMPLATE = os.path.join("C:" + os.sep, "ProgramData", "SOLIDWORKS", "SOLIDWORKS 2024", "templates", "Part.prtdot")
...
def _build_box(sw, path):
    ...
    fm.FeatureExtrusion3(True, False, False, 0, 0, 0.02, 0.0,
...
def main():
    ...
    part_path = os.path.join(tempfile.gettempdir(), "ref_point_pae_%d.SLDPRT" % int(time.time()))
    ...
    if doc is None:
        ...
        return 1
    mod = wrapper_module()
    face_ref = {"normal": [0.0, 0.0, 1.0], "centroid": [0.0, 0.0, 0.02], "area_mm2": 1600.0, "role_hint": "top_face"}
    ...
    if result["ok"]:
        ...
        while feat is not None:
            ...
            if "Point" in str(fname):
                ...
                try: result["feature_type"] = feat.GetTypeName2()
                except: pass
                break
