...
def _capture(fn):
    ...
    except Exception as exc: return {"status": "ERR", "type": type(exc).__name__, "message": str(exc)[:200]}, None
...
def run():
    ...
    if asm_doc is None:
        result["overall"] = "WALL"
        result["interpretation"] = "NewDocument(assembly) None. Assembly creation blocked."
        return result
    ...
    for label, p in [("box1", box1), ("box2", box2)]:
        rec, comp = _capture(lambda pp=p: asm_doc.AddComponent5(pp, 0, "", False, "", 0, 0, 0))
    ...
    if not comps_ok:
        ...
        for label, p in [("box1", box1), ("box2", box2)]:
            ...
            entry = {"label": label, **rec}
            if comp is not None: entry["has_component"] = True
            comps2.append(entry)
    ...
    if idm is not None:
        ...
        try:
            ...
            for mn in ("GetInterferenceCount", "GetInterferences", "SetOptions", "RunInterferenceDetection"):
                try:
                    attr = getattr(idm_t, mn)
                    idm_result[mn] = "callable" if callable(attr) else type(attr).__name__
                except Exception as e:
...
def main():
    ...
    def _safe(o):
        if hasattr(o, "_oleobj_"): return "<COM>"
        return str(o)
