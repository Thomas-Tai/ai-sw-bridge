...
def main() -> int:
    ...
    def args24(end_cond, depth, sd):
        return [
            ...
            False,
            False,
            False,  # 8-11 Dchk1/2 Ddir1/2
            0.0,
            0.0,  # 12-13 Dang1/2
            False,
            False,
            False,
            False,  # 14-17 OffsetReverse1/2, TranslateSurface1/2
            False,  # 18 NormalCut
            True,
            True,
            True,  # 19-21 UseFeatScope, UseAutoSelect, AssemblyFeatureScope
            0,
            0.0,
            False,  # 22-24 T0, StartOffset, FlipStartOffset
        ]
    ...
    test_cases = [
        (
            "FeatureCut4_blind_5mm_Sd_True",
            "FeatureCut4",
            args24(SW_END_COND_BLIND, 0.005, True),
        ),
        (
            "FeatureCut4_blind_5mm_Sd_False",
            "FeatureCut4",
            args24(SW_END_COND_BLIND, 0.005, False),
        ),
        (
            "FeatureCut4_through_all_d0",
            "FeatureCut4",
            args24(SW_END_COND_THROUGH_ALL, 0.0, True),
        ),
        (
            "FeatureCut4_through_next_d0",
            "FeatureCut4",
            args24(SW_END_COND_THROUGH_NEXT, 0.0, True),
        ),
        (
            "FeatureCut3_23args_blind",
            "FeatureCut3",
            args23(SW_END_COND_BLIND, 0.005, True),
        ),
        (
            "FeatureCut_legacy_blind",
            "FeatureCut",
            args23(SW_END_COND_BLIND, 0.005, True),
        ),
    ]
    ...
    for i, (label, method, args) in enumerate(test_cases):
        try:
            ...
            results.append(
                {"label": label, "ok": False, "error": f"sketch setup: {e!r}"}
            )
            continue
        doc.ClearSelection2(True)
        if not doc.SelectByID(sk_name, "SKETCH", 0.0, 0.0, 0.0):
            results.append(
                {"label": label, "ok": False, "error": f"select {sk_name} failed"}
            )
            continue
