"""W70 seat-proof — drawing ``datum_tag`` annotation (LIVE seat).

Drives the PRODUCTION ``commit_drawing`` path with an ``annotations.datum_tag``
block so it seat-proves the real ``_apply_entity_attached_annotation`` handler
(GetVisibleEntities2 -> IEntity.Select2 -> InsertDatumTag2 ->
typed_qi(obj,"IDatumTag").GetAnnotation().SetPosition), NOT a raw probe.
Then reopens the .SLDDRW and counts ``swDatumTag=2`` annotations.

  GREEN := commit_ok AND reopened drawing has >= 1 swDatumTag annotation.

swAnnotationType_e (seat-verified, swconst.tlb): 2 datumTag, 6 note,
7 SFSymbol, 8 weld.

Usage (seat must be UP)::
    C:/Python314/python.exe spikes/v0_2x/spike_datum_tag.py
"""
...
_RESULT_KEY = "datum_tag_annotations"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"w70_{_KIND}_") as tmp:
        ...
        spec = {
            ...
            "views": ["front"],
            "annotations": {
                _KIND: [{"view": "front", "x": 0.10, "y": 0.10}],
            },
        }
        try:
