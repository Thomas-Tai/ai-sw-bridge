...
RESULTS_PATH = Path(__file__).resolve().parents[2] / "spikes" / "v0_2x" / "_results" / "drawing_gtol.json"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="w70_gtol_") as tmp:
        ...
        result["finding"] = f"commit_ok={commit.get('ok')}, gtol_on_reopen={reopen.get('total')}"
        _write(result)
