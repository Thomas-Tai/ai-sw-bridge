...
RESULTS_PATH = (
    ...
    / "spikes" / "v0_2x" / "_results" / "structural_weldment.json"
)
PROFILE = (
...
def run() -> str:
    ...
    base_feat = {
        "profile_path": PROFILE, "configuration": CONFIG, "sketch_name": "PATH",
    }
    ...
    okB, noteB = create_structural_weldment(
        doc, {**base_feat, "connected_segments": "simple_cut",
              "corner_treatment": True, "miter_merge": True}, {})
    ...
    gate("C_zero_ghost_unreachable", okC is False and "connected_segments" in str(noteC),
         str(noteC))
    try:
        sw.CloseAllDocuments(True)
    except Exception:
    ...
    gate("D_missing_profile_failclosed",
         okD is False and "does not exist" in str(noteD), str(noteD))
    try:
        sw.CloseAllDocuments(True)
    except Exception:
        pass
