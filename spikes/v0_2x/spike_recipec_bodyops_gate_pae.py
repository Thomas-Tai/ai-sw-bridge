...
results: dict[str, Any] = {
    "pae": "recipec_bodyops_registry_migration_gate",
    "gates": {},
}
...
def _extrude(
    fm: Any, sm: Any, doc: Any, x0: float, x1: float, merge: bool, name: str
) -> Any:
...
def main() -> int:
    ...
    try:
        ...
        walls_dormant = ("combine" not in HANDLER_REGISTRY) and (
            "split" not in HANDLER_REGISTRY
        )
        in_features = all(
            hasattr(bo, n)
            for n in (
        ...
        if not _build_two_body_part(sw, seed):
            ...
            if not pid:
                gate(
                    "delete_body_lifecycle",
                    False,
                    f"propose failed: {prop.get('error')}",
                )
            else:
