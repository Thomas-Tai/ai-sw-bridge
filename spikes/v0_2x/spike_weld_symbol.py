"""W70 seat-proof — drawing ``weld_symbol`` annotation (LIVE seat).

Drives the PRODUCTION ``commit_drawing`` path with an ``annotations.weld_symbol``
block so it seat-proves the real ``_apply_entity_attached_annotation`` handler
(GetVisibleEntities2 -> IEntity.Select2 -> InsertWeldSymbol3 ->
typed_qi(obj,"IWeldSymbol").GetAnnotation().SetPosition), NOT a raw probe.
Then reopens the .SLDDRW and counts ``swWeldSymbol=8`` annotations.

  GREEN := commit_ok AND reopened drawing has >= 1 swWeldSymbol annotation.

swAnnotationType_e (seat-verified, swconst.tlb): 2 datumTag, 6 note,
7 SFSymbol, 8 weld.

Usage (seat must be UP)::
    C:/Python314/python.exe spikes/v0_2x/spike_weld_symbol.py
"""
...
RESULTS_PATH = Path(__file__).resolve().parents[2] / "spikes" / "v0_2x" / "_results" / "drawing_weld_symbol.json"

_KIND = "weld_symbol"
_SW_TYPE = 8  # swAnnotationType_e.swWeldSymbol
_RESULT_KEY = "weld_symbol_annotations"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"w70_{_KIND}_") as tmp:
        ...
        spec = {
            ...
            "views": ["front"],
            "annotations": {
                _KIND: [{"view": "front", "x": 0.12, "y": 0.12}],
            },
        }
        try:
