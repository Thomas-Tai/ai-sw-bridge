...
RESULTS_PATH = WORKTREE / "spikes" / "v0_2x" / "_results" / "drawing_acquire.json"
...
def run() -> str:
    ...
    gate("drwdot_found", len(drwdots) > 0, f"found {len(drwdots)}: {drwdots}")
    ...
    if template_path is None:
        gate("template_available", False, "NO .drwdot template found — cannot proceed")
        save_results()
    ...
    try:
        ...
        if doc_ok:
            ...
            try:
                ...
                gate(
                    "routeA_qi_idrawingdoc", qi_ok, f"type={type(drawing_doc).__name__}"
                )
    ...
    try:
        ...
        gate(
            "routeC_typelib_scan", len(drawing_methods) > 0, f"found: {drawing_methods}"
        )
    ...
    try:
        ...
        if doc_raw3 and not isinstance(doc_raw3, int):
            ...
            try:
                ...
                liveness["getsheetnames"] = list(sheets) if sheets else []
                gate(
                    "liveness_getsheetnames",
                    True,
            ...
            try:
                ...
                liveness["getfirstview"] = type(view).__name__ if view else None
                gate(
                    "liveness_getfirstview",
                    view is not None,
            ...
            try:
                ...
                liveness["getcurrentsheet"] = type(sheet).__name__ if sheet else None
                gate(
                    "liveness_getcurrentsheet",
                    sheet is not None,
            ...
            try:
                drawing_doc3 = typed_qi(doc_raw3, "IDrawingDoc", module=mod)
                if drawing_doc3:
                    sheets2 = drawing_doc3.GetSheetNames()
                    liveness["typed_getsheetnames"] = list(sheets2) if sheets2 else []
                    gate(
                        "liveness_typed_members",
                        True,
